...
func (t *Trie) Keys() []string {
	...
	keys, _ := t.PrefixSearch("")
	return keys
}

// Performs a fuzzy search against the keys in the trie.
func (t Trie) FuzzySearch(pre string) ([]string, []interface{}) {
	return fuzzycollect(t.Root(), []rune(pre))
}

// Performs a prefix search against the keys in the trie.
func (t Trie) PrefixSearch(pre string) ([]string, []interface{}) {
...
func collect(node *Node) ([]string, []interface{}) {
	var (
		n *Node
		i int
	)
...
func fuzzycollect(node *Node, partial []rune) ([]string, []interface{}) {
	if len(partial) == 0 {
		return collect(node)
	}

...
//	trie.Add("b", nil)
//	trie.Remove("a")
//	if _, b := trie.Find("a"); b {
//		t.Errorf("Expected false got true")
//	}
//	if _, b := trie.Find("b"); !b {
...
func TestPrefixSearch(t *testing.T) {
	...
	for _, test := range tests {
		actual, _ := trie.PrefixSearch(test.pre)
		sort.Strings(actual)
...
func TestFuzzySearch(t *testing.T) {
	...
	for _, test := range tests {
		actual, _ := trie.FuzzySearch(test.partial)
		if len(actual) != test.length {
...
func TestFuzzySearchSorting(t *testing.T) {
	...
	actual, _ := trie.FuzzySearch("fz")
	expected := map[string]bool{}
	expected["bfrza"] = true
	expected["foo/bart/baz.go"] = true
...
func BenchmarkPrefixSearch(b *testing.B) {
	trie := New()
	addFromFile(trie, "test.txt")

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = trie.PrefixSearch("妈")
	}
}

func TestFuzzyWithMetaSearch(t *testing.T) {
	trie := New()
	expected := []string{"苹果 沂水县", "苹果", "大蒜", "大豆", "大比", "哈大哈"}
	for i := range expected {
		trie.Add(expected[i], i)
	}
	_, _ = trie.FuzzySearch("大")

}

func TestPrefixWithMetaSearch(t *testing.T) {
	trie := New()
	expected := []string{"苹果 沂水县", "苹果", "大蒜", "大豆", "大比", "哈大哈"}
	for i := range expected {
		trie.Add(expected[i], i)
	}
	_, _ = trie.PrefixSearch("大")
}

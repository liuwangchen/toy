...
func TestFileLogWriter(t *testing.T) {
	...
	w, _ := NewFileLogWriter(testLogFile, false)
	if w == nil {
...
func TestLogOutput(t *testing.T) {
	...
	writer, _ := NewFileLogWriter(testLogFile, false)
	l.AddFilter("file", FINEST, writer.SetFormat("[%L] %M"))
	defer os.Remove(testLogFile)
...
func TestXMLConfig(t *testing.T) {
	...
	fmt.Fprintln(fd, "    <!--")
	fd.WriteString("       %T - Time (15:04:05 MST)\n")
	fd.WriteString("       %t - Time (15:04)\n")
	fmt.Fprintln(fd, "       %D - Date (2006/01/02)")
	fd.WriteString("       %d - Date (01/02/06)\n")
	...
	fd.WriteString("       Recommended: \"[%D %T] [%L] (%S) %M\"\n")
	fmt.Fprintln(fd, "    -->")
	fd.WriteString("    <property name=\"format\">[%D %T] [%L] (%S) %M</property>\n")
	...
	log := make(Logger)
	err = log.LoadConfigurationFromFile(configfile)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Move XML log file
	err = os.Rename(configfile, "examples/"+configfile) // Keep this so that an example with the documentation is available
	if err != nil {
		t.Error(err)
	}
}
...
func BenchmarkFileLog(b *testing.B) {
	sl := make(Logger)
	b.StopTimer()
	writer, _ := NewFileLogWriter("benchlog.log", false)
	sl.AddFilter("file", INFO, writer)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		sl.Log(WARNING, "here", "This is a log message")
	}

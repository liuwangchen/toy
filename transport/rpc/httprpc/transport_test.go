...
func TestTransport_Kind(t *testing.T) {
	o := &Transport{}
	if !reflect.DeepEqual(rpc.KindHTTP, o.Kind()) {
		t.Errorf("expect %v, got %v", rpc.KindHTTP, o.Kind())
	}
}
...
func TestSetOperation(t *testing.T) {
	tr := &Transport{}
	ctx := rpc.NewServerContext(context.Background(), tr)
	SetOperation(ctx, "goctopus")
	if !reflect.DeepEqual(tr.operation, "goctopus") {
		t.Errorf("expect %v, got %v", "goctopus", tr.operation)
	}
}

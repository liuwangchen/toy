...
func TestContextHeader(t *testing.T) {
	w := wrapper{
		router: nil,
		req:    &http.Request{Header: map[string][]string{"name": {"goctopus"}}},
		res:    nil,
		w:      responseWriter{},
	}
	h := w.Header()
	if !reflect.DeepEqual(h, http.Header{"name": {"goctopus"}}) {
		t.Errorf("expected %v, got %v", http.Header{"name": {"goctopus"}}, h)
	}
}
...
func TestContextForm(t *testing.T) {
	w := wrapper{
		router: nil,
		req:    &http.Request{Header: map[string][]string{"name": {"goctopus"}}, Method: "POST"},
		res:    nil,
		w:      responseWriter{},
	}
	form := w.Form()
	...
	w = wrapper{
		router: nil,
		req:    &http.Request{Form: map[string][]string{"name": {"goctopus"}}},
		res:    nil,
		w:      responseWriter{},
	}
	form = w.Form()
	if !reflect.DeepEqual(form, url.Values{"name": {"goctopus"}}) {
		t.Errorf("expected %v, got %v", url.Values{"name": {"goctopus"}}, form)
	}
}
...
func TestContextQuery(t *testing.T) {
	w := wrapper{
		router: nil,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "github.com", Path: "go-goctopus/goctopus", RawQuery: "page=1"}, Method: "POST"},
		res:    nil,
		w:      responseWriter{},
	}
	q := w.Query()
...
func TestContextResponse(t *testing.T) {
	...
	w := wrapper{
		router: &Router{srv: &ServerConn{enc: DefaultResponseEncoder}},
		req:    &http.Request{Method: "POST"},
		res:    res,
...
func TestContextBindQuery(t *testing.T) {
	w := wrapper{
		router: nil,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "go-goctopus-dev", RawQuery: "page=2"}},
		res:    nil,
		w:      responseWriter{},
	}
...
func TestContextBindForm(t *testing.T) {
	w := wrapper{
		router: nil,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "go-goctopus-dev"}, Form: map[string][]string{"page": {"2"}}},
		res:    nil,
		w:      responseWriter{},
	}

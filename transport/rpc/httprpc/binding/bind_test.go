...
func TestBindQuery(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "test",
			args: args{
				vars:   map[string][]string{"name": {"goctopus"}, "url": {"https://go-goctopus.dev/"}},
				target: &p1,
			},
			wantErr: false,
			want:    &TestBind{"goctopus", "https://go-goctopus.dev/"},
		},
	}
		...
		},
		{
			...
			args: args{
				req: &http.Request{
					...
					Body:   io.NopCloser(strings.NewReader("name=goctopus&url=https://go-goctopus.dev/")),
				},
				target: &p1,
			},
			wantErr: false,
